/* This File is based on intobject.c from CPython 2.7.3 and 2.7.4 release.
...
typedef struct _JyIntObject JyIntObject;

#define BLOCK_SIZE	  1000	/* 1K less typical malloc overhead */
#define BHEAD_SIZE	  8	   /* Enough for a 64-bit pointer */
#define N_INTOBJECTS	((BLOCK_SIZE - BHEAD_SIZE) / (sizeof(JyIntObject)))
...
fill_free_list(void)
{
	...
	while (--q > p)
	{
		...
		//printf("test: %u\n", Py_TYPE(FROM_JY_NO_GC(q)));
	}
	Py_TYPE(FROM_JY_NO_GC(q)) = NULL;
	Jy_InitImmutable((JyObject*) q);
	return (PyIntObject*) FROM_JY_NO_GC(p + N_INTOBJECTS - 1);
//	printf("test2 %u\n", (p + N_INTOBJECTS - 1));
//	PyIntObject* er = (PyIntObject*) FROM_JY_NO_GC(p + N_INTOBJECTS - 1);
//	printf("test3 %u\n", er);
//	puts("Free List:");
//	printf("(size of JyIntObject: %i)\n", sizeof(JyIntObject));
//	printf("(size of JyObject: %i)\n", sizeof(JyObject));
//	printf("(size of PyIntObject: %i)\n", sizeof(PyIntObject));
//	int i = 0;
//	PyIntObject* er2 = er;
//	while(Py_TYPE(er2) != NULL)
//	{
//		printf("%i: address: %u\n", i++, (long) er2);
//		er2 = (PyIntObject*) Py_TYPE(er2);
//		printf("next: %u\n", (long) er2);
//	}
////	er2 = (PyIntObject*) Py_TYPE(er2);
////	printf("%i: address: %u", i, (long) er2);
//	return er;
}
...
PyInt_FromLong(long ival)
{
	...
	if (-NSMALLNEGINTS <= ival && ival < NSMALLPOSINTS) {
		//v = (PyIntObject*) FROM_JY_NO_GC(small_ints[ival + NSMALLNEGINTS]);
...
PyInt_Fini(void)
{
	...
	//if (!Py_VerboseFlag)
	env();
	jint pyVerbose = (*env)->CallStaticIntMethod(env, JyNIClass, JyNIGetDLVerbose);
	if (!pyVerbose)
		return;
