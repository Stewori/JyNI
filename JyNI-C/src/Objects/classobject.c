/* This File is based on classobject.c from CPython 2.7.5 release.
 ...
 *
 * JyNI note: Maybe a smaller number than 256 would be sufficient here, since
 * it is only needed for extensions.
 */
...
class_getattr(register PyClassObject *op, PyObject *name)
{
	...
	if (sname[0] == '_' && sname[1] == '_') {
		if (strcmp(sname, "__dict__") == 0) {
			//Jython doesn't support restricted execution mode
...
instance_ass_slice(PyInstanceObject *inst, Py_ssize_t i, Py_ssize_t j, PyObject *value)
{
	...
	if (value == NULL) {
		...
		if (func == NULL) {
			...
			if (func == NULL)
				return -1;
			//JyNI todo: provide Py_BuildValue
...
init_name_op(void)
{
	...
	//JyNI-note: On first look this looks alarming, but no actual PyObjects
	//are allocated, but only pointers - so no danger of segfaults with AS_JY or FROM_JY.
