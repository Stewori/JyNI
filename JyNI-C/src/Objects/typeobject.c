/* This File is based on typeobject.c from CPython 2.7.3 release.
...
//	Py_ssize_t i, n;
	...
	//JyNI note: Don't forget to re-include this, when subclassing stuff is supported:
...
//		env(NULL);
//		jstring jname = (*env)->CallObjectMethod(env, JyNI_JythonPyObject_FromPyObject(type), pyTypeGetName);
//		jputs("got type jname");
//		if (!jname) jputs("is NULL");
//
...
type_module(PyTypeObject *type, void *context)
{
	...
	if (type->tp_flags & Py_TPFLAGS_HEAPTYPE) {
//		jputs("heaptype");
//		jputs(PyString_AS_STRING(type_name(type, context)));
//		jputs("heaptype2");
...
static PyGetSetDef type_getsets[] = {
	...
	{"__bases__", (getter)type_get_bases, NULL, NULL},//(setter)type_set_bases, NULL},
...
//	jputs(type->tp_name);
	...
	} else
	{
		...
		if (tme) {
			return JyNI_AllocSubtypeVar(type, tme, nitems);
		} else {
...
//		jputsLong(JyNI_GetJythonDelegate(metatype));
		...
		if (JyObject_IS_INITIALIZED(mtjy))
		{
			...
			//	jputsLong(__LINE__);
				...
				dct = JyNI_JythonPyObject_FromPyObject(dict);
				/*
...
//		}
		...
		if ((type->tp_flags & Py_TPFLAGS_HAVE_GC) ==
			...
			COPYSLOT(tp_free);
				 ...
				 type->tp_free == NULL &&
				 base->tp_free == PyObject_Free) { //JyNI: originally _PyObject_Del
...
//	   the tp_doc slot.
	//puts("doc slot...");
	//if (type->tp_dict) puts("type dict is nonzero");
	//else puts("type dict is NULL");
