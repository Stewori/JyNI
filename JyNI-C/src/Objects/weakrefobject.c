/* This File is based on weakrefobject.c from CPython 2.7.10 release.
...
/*
 * JyNI-Note:
 * This only counts native reference objects. We keep it that way,
 * because this method is just used for memory clean-up internally
 * by weakref. So adding non-native refcount to it would be awkward.
 */
Py_ssize_t
...
new_weakref(PyObject *ob, PyObject *callback, PyTypeObject* refType)
{
	PyWeakReference *result;

	//result = PyObject_GC_New(PyWeakReference, &_PyWeakref_RefType);
	result = PyObject_GC_New(PyWeakReference, refType);
	if (result) {
...
//	jputsLong(__LINE__);
	if (result != NULL)
		...
		/* Note: new_weakref() can trigger cyclic GC, so the weakref
		   ...
		   so we need to compute these values again before we use
		   them. */
		result = new_weakref(ob, callback, &_PyWeakref_RefType);
		if (result != NULL) {
			get_basic_refs(*list, &ref, &proxy);
			if (callback == NULL) {
				if (ref == NULL)
