...
{
	puts("JyNI-Note: We must really fix exception handling soon!");
//	PyObject *tup;
 ...
 * mutates the table.
 */
/* JyNI-note: This implementation is tedious, since we have to create
 * setentry struct for every call. To keep track of them, reuse them and deallocate them
 * with the set we would track them in JyAttributes. This is slow and inefficient, so don't
 * use this method for large sets!
 */
/* For now, we try to live with it commented out
typedef struct {
	Py_ssize_t pos;      // cached hash code for the entry key
	setentry entry;
} setentryLookup;

static setentryLookup* getEntryLookup(PySetObject *so, Py_ssize_t *pos_ptr)
{
	JyObject* jy = AS_JY((PyObject*) so);
	JyAttributeElement* elem = (JyAttributeElement*) JyNI_GetJyAttribute(jy, JyAttributeSetEntry);
	if (elem == NULL) return NULL;
	setentryLookup* sel;
	while (elem != NULL)
	{
		sel = (setentryLookup*) elem->value;
		if (sel->pos == *pos_ptr) return sel;
		elem = elem->next;
	}
	return NULL;
}

static void addEntryLookup(PySetObject *so, setentryLookup* lookup)
{
	JyObject* jy = AS_JY((PyObject*) so);
	JyAttributeElement* elem = (JyAttributeElement*) JyNI_GetJyAttribute(jy, JyAttributeSetEntry);
	JyAttributeElement* nelem = malloc(sizeof(JyAttributeElement));
	nelem->value = lookup;
	if (elem == NULL)
	{
		JyNI_AddJyAttributeWithFlags(jy, JyAttributeSetEntry, nelem,
				JY_ATTR_OWNS_VALUE_FLAG_MASK | JY_ATTR_VAR_SIZE_FLAG_MASK);
	} else
	{
		nelem->next = elem->next;
		elem->next = nelem;
	}
}
...
set_next(PySetObject *so, Py_ssize_t *pos_ptr, setentry **entry_ptr)
{
	...
	jobject res = (*env)->CallStaticObjectMethod(env, JyNIClass,
		JyNIGetPySet_Next, JyNI_JythonPyObject_FromPyObject((PyObject*) so), *pos_ptr);
	...
	setentryLookup* lookup = getEntryLookup(so, pos_ptr);
	if (lookup == NULL)
	{
		lookup = malloc(sizeof(setentryLookup));
		lookup->pos = *pos_ptr;
		addEntryLookup(so, lookup);
	}
	setentry* er = lookup->entry;

	if (k != NULL) er->key = (PyObject*) k;
	else er->key = JyNI_PyObject_FromJythonPyObject(k2);
	if (k2 != NULL)
		er->hash = (*env)->CallIntMethod(env, k2, pyObjectHashCode);
...
set_tp_print(PySetObject *so, FILE *fp, int flags)
{
	...
	char *separator = ", ";
	jboolean status;
	jobject threadState, set = JyNI_JythonPyObject_FromPyObject((PyObject*) so);
	env(-1);
	threadState = (*env)->CallStaticObjectMethod(env, pyPyClass, pyPy_getThreadState);

	//int status = Py_ReprEnter((PyObject*)so);
	status = (*env)->CallBooleanMethod(env, threadState, pyThreadState_enterRepr, set);
	...
	while (_PySet_Next((PyObject*) so, &pos, &key)) {
		//Py_BEGIN_ALLOW_THREADS
		fputs(emit, fp);
		//Py_END_ALLOW_THREADS
		emit = separator;
...
//JyNI note: This method seems to be not needed by our implementation.
//static int
...
set_pop(PySetObject *so)
{
	if (PySet_GET_SIZE(so) == 0) {
		...
		er = (*env)->CallObjectMethod(env, JyNI_JythonPyObject_FromPyObject((PyObject*) so),
				pySet_set_pop);
		if (er == NULL) {
...
//JyNI note: For now, we try to live without this method.
...
//		return JyNI_PyObject_FromJythonPyObject(
//			(*env)->CallObjectMethod(env, JyNI_JythonPyObject_FromPyObject((PyObject*) so),
//				pyObject__or__, JyNI_JythonPyObject_FromPyObject(other)));
//}
	PySetObject *result;
...
set_intersection_update(PySetObject *so, PyObject *other)
{
	jobject args;
	env(NULL);
	args = (*env)->NewObjectArray(env, 1, pyObjectClass, JyNI_JythonPyObject_FromPyObject(other));
...
//	return JyNI_PyObject_FromJythonPyObject(
//		(*env)->CallObjectMethod(env, JyNI_JythonPyObject_FromPyObject((PyObject*) so),
//			pyObject__and__, JyNI_JythonPyObject_FromPyObject(other)));
//}
	if (!PyAnySet_Check(so) || !PyAnySet_Check(other)) {
		Py_INCREF(Py_NotImplemented);
		return Py_NotImplemented;
...
//JyNI-note: For now we don't implement set_sizeof, since we can't obtain
//the correct result anyway. However, a chance to get the size of the set
//in JVM-memory is Instrumentation.getObjectSize. Maybe we will implement it
//based on that one in the future.
//static PyObject *
...
//	env(-1);
//	(*env)->CallVoidMethod(env, JyNI_JythonPyObject_FromPyObject(set), pyBaseSetClear);
//	((PySetObject *) set)->fill = 0;
//	((PySetObject *) set)->used = 0;
//	return 0;
}
...
//	env(-1);
//	return (*env)->CallBooleanMethod(env, JyNI_JythonPyObject_FromPyObject(anyset),
//		pyBaseSetContains, JyNI_JythonPyObject_FromPyObject(key));
...
//	env(-1);
//	jboolean er = (*env)->CallBooleanMethod(env, JyNI_JythonPyObject_FromPyObject(anyset),
//		pyBaseSetAdd, JyNI_JythonPyObject_FromPyObject(key));
//	if (er) ++((PySetObject*) anyset)->used; //should be done by JySet
//	return er;
...
_PySet_Next(PyObject *set, Py_ssize_t *pos, PyObject **key)
{
	...
	if (!PyAnySet_Check(set)) {
		...
		jlong k;
		env(-1);
		res = (*env)->CallStaticObjectMethod(env, JyNIClass,
				JyNI_getPySet_Next, JyNI_JythonPyObject_FromPyObject(set), *pos);
		if (res == NULL) return 0;
		k = (*env)->GetLongField(env, res, JyNISetNextResult_keyHandleField);
...
_PySet_NextEntry(PyObject *set, Py_ssize_t *pos, PyObject **key, long *hash)
{
	...
	if (!PyAnySet_Check(set)) {
		...
		if (res == NULL) return 0;
		k = (*env)->GetLongField(env, res, JyNISetNextResult_keyHandleField);
		k2 = (*env)->GetObjectField(env, res, JyNISetNextResult_keyField);
		if (k != NULL) *key = (PyObject*) k;
...
//	}
//	env(NULL);
//	//JyObject* jy = AS_JY(set);
//	jobject er = (*env)->CallStaticObjectMethod(env, JyNIClass, JyNIPySet_pop, JyNI_JythonPyObject_FromPyObject(set));//jy->jy);
//	if (er == NULL) {
//		PyErr_SetString(PyExc_KeyError, "pop from an empty set (corrupted set or bug, since size > 0)");
//		return NULL;
//	}
...
//	env(-1);
//	jobject jset = JyNI_JythonPyObject_FromPyObject(set);
//	(*env)->CallVoidMethod(env, jset,
//		pyBaseSet_update, JyNI_JythonPyObject_FromPyObject(iterable));
//	((PySetObject*) set)->used = (*env)->CallIntMethod(env, jset, pyBaseSetSize);
//	return 0;
