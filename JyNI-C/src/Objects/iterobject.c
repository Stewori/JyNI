/* This File is based on iterobject.c from CPython 2.7.11 release.
...
{
	JyNIDebugOp(JY_NATIVE_FINALIZE, it, -1);
	PyObject_Free(it);
...
//	if (it->it_seq) {
	if (seq) {
	    seqsize = PySequence_Size(seq);//it->it_seq);
	    Py_XDECREF(seq);
	    if (seqsize == -1)
	        return NULL;
...
PyTypeObject PySeqIter_Type = {
	...
	0,                                          /* tp_doc */
	0,//(traverseproc)iter_traverse,            /* tp_traverse */
	0,                                          /* tp_clear */
	0,                                          /* tp_richcompare */
	0,                                          /* tp_weaklistoffset */
