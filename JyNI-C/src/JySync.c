...
jboolean isPreAllocatedJythonString(jobject obj, char value)//jchar value)
{
	if (value >= LETTERCHAR_MAXJYTHON)
		...
		jarray scache;
		jobject cachedString;
		env(JNI_FALSE);
		scache = (*env)->GetStaticObjectField(env, pyPyClass, pyPy_lettersField);
		cachedString = (*env)->GetObjectArrayElement(env, scache, (jint) value);
...
PyObject* JySync_Init_PyString_From_JyString(jobject src, PyTypeObject* nonNativeSubtype)
{
	...
	size_t len;
	env(NULL);
	jstr = (*env)->CallObjectMethod(env, src, pyObject_asString);
...
jobject JySync_Init_JyString_From_PyString(PyObject* src, jclass subtype)
{
	...
	jstring jstr;
	env(NULL);
	jstr = (*env)->NewStringUTF(env, PyString_AS_STRING(src));
...
{
	jstring jstr, charsetName;
	jobject stringByteArray;
	jbyte* stringBytes;
	jsize len;
	...
	jstr = (*env)->CallObjectMethod(env, src, pyObject_asString);
	charsetName = (*env)->NewStringUTF(env, "UTF-16BE");
	stringByteArray = (*env)->CallObjectMethod(env, jstr, string_getBytes, charsetName);
	stringBytes = (*env)->GetByteArrayElements(env, stringByteArray, NULL);
	len = (*env)->GetArrayLength(env, stringByteArray);
...
static inline jboolean isPreAllocatedJythonInt(jobject obj, jint value)
{
	if (value < -NSMALLNEGINTSJYTHON || value >= NSMALLPOSINTSJYTHON)
		...
		env(JNI_FALSE);
		icache = (*env)->GetStaticObjectField(env, pyPyClass, pyPy_integerCacheField);
		cachedInt = (*env)->GetObjectArrayElement(env, icache, value+NSMALLNEGINTSJYTHON);
...
PyObject* JySync_Init_PyInt_From_JyInt(jobject src, PyTypeObject* nonNativeSubtype)
{
	...
	value = (*env)->CallIntMethod(env, src, pyInt_getValue);
...
jobject JySync_Init_JyList_From_PyList(PyObject* src, jclass subtype)
{
	...
	jyList = (*env)->NewLocalRef(env, (jobject) JyObject_GetJyGCHead(src, AS_JY_WITH_GC(src)));
...
PyObject* JySync_Init_PyList_From_JyList(jobject src, PyTypeObject* nonNativeSubtype)
{
	...
	//much lookup is done twice here.
	{
		...
		jsize i, size = (*env)->GetArrayLength(env, handles);
		PyObject* v, * op = PyList_New((Py_ssize_t) size);
		...
		arr = (*env)->GetLongArrayElements(env, handles, NULL);
		for (i = 0; i < size; ++i)
		{
			v = (PyObject*) arr[i];
	...
	//JyNI_PyObject_FromJythonPyObject(jobject jythonPyObject, jboolean lookupNative, jboolean checkCPeer, jboolean checkForType)
		...
		//JyNI_GC_ExploreObject should have initialized JyGCHead to a JyList-object.
		jyList = (*env)->NewLocalRef(env, (jobject) JyObject_GetJyGCHead(op, AS_JY_WITH_GC(op)));
		if (!jyList || (*env)->IsSameObject(env, jyList, NULL)) {
			jputs("This should not happen! JyList gc-head was deleted after explore.");
			jputs(__FUNCTION__);
			if (!jyList) jputs("truely NULL");
...
//	jy->flags |= JY_INITIALIZED_FLAG_MASK;
	...
	_set = (*env)->GetObjectField(env, src, pyBaseSet__setField);
	jySet = (*env)->NewObject(env, JySetClass, JySet_fromBackendHandleConstructor, _set, (jlong) so);
...
PyObject* JySync_Init_PyClass_From_JyClass(jobject src, PyTypeObject* nonNativeSubtype)
{
	...
	nm = (*env)->GetObjectField(env, src, pyClass___name__Field);
...
jobject JySync_Init_JyProperty_From_PyProperty(PyObject* src, jclass subtype)
{
	...
	result = (*env)->NewObject(env, pyPropertyClass, pyProperty_Constructor);
...
//	jputs(__FUNCTION__);
	...
	//	puts(cName);
		...
		if (jtmp)
		{
			cstr_decl_global2(cDoc);
			global_cstr_from_jstring(cDoc, jtmp);
			mdef->ml_doc = cDoc;
...
//func_code, func_globals, func_module, func_defaults, func_closure
	jtmp = (*env)->GetObjectField(env, src, pyFunction___code__Field);
	((PyFunctionObject*) dest)->func_code = JyNI_PyObject_FromJythonPyObject(jtmp);

	jtmp = (*env)->CallObjectMethod(env, src, pyFunction_getFuncGlobals);
	((PyFunctionObject*) dest)->func_globals = JyNI_PyObject_FromJythonPyObject(jtmp);

	jtmp = (*env)->GetObjectField(env, src, pyFunction___module__Field);
	((PyFunctionObject*) dest)->func_module = JyNI_PyObject_FromJythonPyObject(jtmp);

	jtmp = (*env)->CallObjectMethod(env, src, pyFunction_getFuncDefaults);
	((PyFunctionObject*) dest)->func_defaults = JyNI_PyObject_FromJythonPyObject(jtmp);

	jtmp = (*env)->GetObjectField(env, src, pyFunction___closure__Field);
	((PyFunctionObject*) dest)->func_closure = JyNI_PyObject_FromJythonPyObject(jtmp);

	jtmp = (*env)->GetObjectField(env, src, pyFunction___name__Field);
	cstr_from_jstring(cName, jtmp);
...
//	JyNI_jprintJ(src);
	...
	//	jputsLong(tp);
		...
		if (!tp->tp_mro)
		{
			...
			mro = JySync_Init_PyTuple_From_JyTupleForMRO(jtmp);
