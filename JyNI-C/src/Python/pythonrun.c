/* This File is based on pythonrun.c from CPython 2.7.6 release.
 ...
 *
 * JyNI-note: In JyNI, this file is mainly a stub. JyNI's top-level
 * routines are suited in JyNI.c.
 */
...
{
	/* JyNI is initialized on JNI startup.
	 * So this should be always true, if an
	 * extension asks.
	 */
...
{
	/* JyNI-note:
	 * Here happens nothing for now.
	 * We mainly implemented this to
	 * have one potential symbol less
	 * that could be missing on extension
	 * load.
	 */
}
...
//	}
	...
	if (exception == NULL)
		return;
	jobject jexc = JyNI_JythonPyObject_FromPyObject(exception);
	jobject jexcval = JyNI_JythonPyObject_FromPyObject(v);
	jobject jexctb = JyNI_JythonPyObject_FromPyObject(tb);
