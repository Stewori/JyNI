/* This File is based on _codecsmodule.c from CPython 2.7.13 release.
...
{
	PyObject* pyinput, *pyerrors, *pyres, *pyfinal, *pyargs;
	jobject res;
	RE_ENTER_JyNI

	pyinput = JyNI_PyObject_FromJythonPyObject(input);
	pyerrors = JyNI_PyObject_FromJythonPyObject(errors);
	pyfinal = JyNI_PyObject_FromJythonPyObject(final);
	pyargs = PyTuple_New(3);
	// The following calls steal references, so later no DECREF is required for the items.
	PyTuple_SET_ITEM(pyargs, 0, pyinput);
	PyTuple_SET_ITEM(pyargs, 1, pyerrors);
	PyTuple_SET_ITEM(pyargs, 2, pyfinal);
	pyres = mbcs_decode(NULL, pyargs);
	res = JyNI_JythonPyObject_FromPyObject(pyres);
	Py_XDECREF(pyargs);
	// no decref for pyinput, pyerrors, pyfinal: Indirectly done by Py_XDECREF(pyargs)
	Py_XDECREF(pyres);
...
jobject JyNI_mbcs_decode(jobject input, jobject errors, jobject final, jlong tstate)
{
	jputs("JyNI error: mbcs_decode is only available on Windows.");
	return NULL;
}
...
{
		PyObject* pyinput, *pyerrors, *pyres, *pyargs;
		jobject res;
		RE_ENTER_JyNI
...
jobject JyNI_mbcs_encode(jobject input, jobject errors, jlong tstate)
{
	jputs("JyNI error: mbcs_encode is only available on Windows.");
	return NULL;
}
...
jobject JyNI_mbcs_encode(jobject input, jobject errors, jlong tstate)
{
	jputs("JyNI error: mbcs_encode requires unicode support.");
	return NULL;
}

jobject JyNI_mbcs_decode(jobject input, jobject errors, jobject final, jlong tstate)
{
	jputs("JyNI error: mbcs_decode requires unicode support.");
	return NULL;
}
