/* This File is based on floatobject.c from CPython 2.7.4 release.
...
struct _JyFloatObject {
	JyObject jy;
	PyFloatObject pyFloat;
};
typedef struct _JyFloatObject JyFloatObject;
...
PyFloat_ClearFreeList(void)
{
	...
	while (list != NULL) {
		...
		for (i = 0, p = &list->objects[0];
			 i < N_FLOATOBJECTS;
			 i++, p++) {
			if (PyFloat_CheckExact(FROM_JY_NO_GC(p)) && p->pyFloat.ob_refcnt != 0) //Py_REFCNT(p) != 0)
				u++;
		}
		next = list->next;
		if (u) {
			...
			for (i = 0, p = &list->objects[0];
				...
				if (!PyFloat_CheckExact(FROM_JY_NO_GC(p)) ||
						p->pyFloat.ob_refcnt ==0) { //Py_REFCNT(p) == 0) {
	...
	if (pyVerbose > 1) {
		...
		while (list != NULL) {
			for (i = 0, p = &list->objects[0];
				 i < N_FLOATOBJECTS;
				 i++, p++) {
				if (PyFloat_CheckExact(FROM_JY_NO_GC(p)) && p->pyFloat.ob_refcnt != 0) {
					//Py_REFCNT(p) != 0) {
				 ...
				 "#   <float at %p, refcnt=%ld, val=%s>\n",
				 	 	 	 	 FROM_JY_NO_GC(p), //(long)Py_REFCNT(p),
